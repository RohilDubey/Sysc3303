...
public class TFTPServerListener  extends Thread {
    ...
    public void run(){
        ...
        for (;;){ 
            ...
            DatagramPacket receivePacketPrevious = new DatagramPacket(data, data.length);
            ...
            while(timeout){
	            try {
	                ...
	                timeout=false;
	                if(receivePacket.getData()[1]== 2 && receivePacketPrevious.getData()[1] == 2 &&(receivePacket.getData()==receivePacketPrevious.getData())){
	                	                	
	                	duplicate=true;
	                }
	                receivePacketPrevious = receivePacket;
            ...
            Thread handler1=new Thread(new TFTPServerHandler(receivePacket));
            handler1.start();
            }
            handler.start();
            timeout=true; 
